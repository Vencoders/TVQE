...
def receive_arg():
    ...
    parser.add_argument(
        '--opt_path', type=str, default='option_QP32.yml',
...
def main():
    ...
    # model = VRT(upscale=1, img_size=[2,32,32], window_size=[6,8,8], depths=[4,4,4,4,4,4,4, 2,2, 2,2],
    #                 indep_reconsts=[9,10], embed_dims=[48,48,48,48,48,48,48, 60,60, 60,60],
    #                 num_heads=[3,3,3,3,3,3,3, 3,3, 3,3], pa_frames=2, deformable_groups=16)
    # model = SwinIR(upscale=1, in_chans=7, img_size=64, window_size=1,
    #                 img_range=1., depths=[4, 4, 4, 4], embed_dim=32, num_heads=[4, 4, 4, 4],
    #                 mlp_ratio=1, upsampler='', resi_connection='1conv')
    # model = SWINMFVQE(opts_dict=opts_dict['network'])
    ...
    # ckp_path = opts_dict['network']['swinunet']['load_path']
    ...
    #     "Not implemented."
    if opts_dict['train']['loss'].pop('type') == 'CharbonnierLoss':
        print("CharbonnierLoss")
        loss_func = utils.CharbonnierLoss(**opts_dict['train']['loss'])
    elif opts_dict['train']['loss']['type'] == 'CharbFFTLoss':
        print("CharbFFTLoss")
        loss_func = utils.CharbFFTLoss()
    elif opts_dict['train']['loss']['type'] == 'FFTLoss':
        print("FFTLoss")
        loss_func = utils.FFTLoss()
    ...
    for current_epoch in range(start_epoch, num_epoch + 1):
        ...
        while train_data is not None:
            ...
            # get data
            gt_data = train_data['gt'].to(rank)  # (B [RGB] H W) (32,15,1,96,96)
            lq_data = train_data['lq'].to(rank)  # (B T [RGB] H W)(32,15,1,96,96)
            # print(gt_data.size(), "vs", lq_data.size())
            b, _, c, _, _  = lq_data.shape
            input_data = torch.cat(
                [lq_data[:,:,i,...] for i in range(c)],
                dim=1
                )  # B [R1 ... R7 G1 ... G7 B1 ... B7] H W  (32,15,96,96)
            enhanced_data = model(input_data) #(32,1,96,96)
            ...
            if ((num_iter_accum % interval_val == 0) or \
                (num_iter_accum == num_iter)) and (rank == 0) and (num_iter_accum >=700000):
            # if ((num_iter_accum % interval_val == 0) or \
            #     (num_iter_accum == num_iter)) and (rank == 0):
                ...
                msg = (
                    "> model saved at {:s}\n"
                    "> ave val per: [{:.3f}] {:s} delta PSNR: [{:.3f}] {:s} "
                    ).format(
                        checkpoint_save_path, ave_per, unit, (ave_per-ave_performance), unit
                        )
                print(msg)
                log_fp.write(msg + '\n')
                log_fp.flush()
