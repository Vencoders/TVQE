...
from model.PixelUnshuffle import PixelUnshuffle
...
class Attention_axial(nn.Module):
    ...
    """
    def __init__(self, dim, idx, split_size=7, dim_out=None, num_heads=6, attn_drop=0., proj_drop=0., qk_scale=None, position_bias=True):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0).cuda()
...
class CATB_axial(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # query = F.pad(q, (0, 0, 0, pad_W, 0, pad_H))
...
class CAT_Unet(nn.Module):
    def __init__(self,
        ...
        self.decoder_level1 = nn.ModuleList([CATB_axial(dim=dim,
                                                           num_heads=num_heads[1],
                                                           # reso=img_size,
                                                           mlp_ratio=mlp_ratio,
                                                           qkv_bias=qkv_bias,
    ...
    def forward(self, inp_img):
        ...
        for layer in self.encoder_level1:
            out_enc_level1 = layer(out_enc_level1, [H//2, W//2])


        inp_enc_level2 = self.down1_2(out_enc_level1, H//2, W//2)
        out_enc_level2 = inp_enc_level2
        for layer in self.encoder_level2:
            out_enc_level2 = layer(out_enc_level2, [H//4, W//4])

        inp_enc_level3 = self.down2_3(out_enc_level2, H//4, W//4)
        out_enc_level3 = inp_enc_level3
        for layer in self.encoder_level3:
            latent = layer(out_enc_level3, [H//8, W//8])


        for layer in self.decoder_level3:
            out_dec_level3 = layer(latent, [H//8, W//8])
        inp_dec_level2 = torch.cat([out_dec_level3, latent], 2)
        inp_dec_level2 = self.up3_2(inp_dec_level2, H//8, W//8)
        ...
        for layer in self.decoder_level2:
            out_dec_level2 = layer(out_dec_level2, [H//4, W//4])
        inp_dec_level1 = torch.cat([out_dec_level2, out_enc_level2], 2)
        inp_dec_level1 = self.up2_1(inp_dec_level1, H // 4, W // 4)
        ...
        for layer in self.decoder_level1:
            out_dec_level1 = layer(out_dec_level1, [H // 2, W // 2])
        inp_dec_level0 = torch.cat([out_dec_level1, out_enc_level1], 2)
        result = self.up0(inp_dec_level0)
