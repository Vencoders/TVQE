...
parser.add_argument(
    '--opt_path', type=str, default='option_R0_mfqev2_1G.yml',
    help='Path to option YAML file.'
    )
args = parser.parse_args()

yml_path = args.opt_path
radius = 0  # must be 3!!! otherwise, you should change dataset.py
...
def create_lmdb_for_mfqev2():
    ...
    for iter_vid in range(len(lq_video_list)):
        ...
        # print("FRM_SEQ=",frm_seq)
        for iter_seq in range(len(frm_seq)):
            key_list.append('{:03d}/{:03d}/'.format(iter_vid+1,iter_seq+1))
            video_path_list.append(lq_video_list[iter_vid])
            # print(lq_video_list[iter_vid],'PP')
            index_frame_list.append(frm_seq[iter_seq][0])
            # print(frm_seq[iter_seq][0])
    ...
    # sym-link
    if not op.exists('data/MFQEv2_RFDA'):
        if not op.exists('data/'):
            os.system("mkdir data/")
        os.system(f"ln -s {root_dir} ./data/MFQEv2_RFDA")
        print("Sym-linking done.")
    else:
        print("data/MFQEv2_RFDA already exists.")
