...
def main():
    ...
    with torch.no_grad():
        while val_data is not None:
            ...
            pbar.set_description(
                "{:s}: [{:.3f}] {:s} "
                    .format(name_vid, batch_ori, unit)
            )
            pbar.update()
    ...
    print("PVD",PVDSD(PSNRS)[0],"STD",PVDSD(PSNRS)[1])
    log_fp.write("PVD:"+str(PVDSD(PSNRS)[0])+"STD: "+str(PVDSD(PSNRS)[1]))
    log_fp.close()
