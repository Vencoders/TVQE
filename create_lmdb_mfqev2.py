...
def create_lmdb_for_mfqev2():
    ...
    gt_video_list1 = []  # 创建一个列表让其存储出现错误的数据下标
    for i in gt_video_list:  # 此时i就是单独的项了如 '1'
        ldv = i.split('.')[-2].split('/')[-1].split('_')[-3]
        if not ldv.startswith('track'):
            gt_video_list1.append(i)

    #lq_video_list = sorted(glob.glob(op.join(lq_dir, '*.yuv')))
    lq_video_list = [op.join(
        lq_dir, 
        gt_video_path.split('/')[-1]
        ) for gt_video_path in gt_video_list1]
    msg = f'> {len(gt_video_list1)} videos found.'
    print(msg)
    ...
    # sym-link
    if not op.exists('data/LDV_1MFQE_qp22_R3'):
        if not op.exists('data/'):
            os.system("mkdir data/")
        os.system(f"ln -s {root_dir} ./data/LDV_1MFQE_qp22_R3")
        print("Sym-linking done.")
    else:
        print("data/LDV_1MFQE_qp22_R3 already exists.")
